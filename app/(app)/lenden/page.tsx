...
export default async function DashboardPage() {
  const [loans, borrowers, allPayments, allContributions, deposits, coreFriends, currentProfile, transfers] =
    await Promise.all([
      getLoans(),
      getBorrowers(),
      getAllPayments(),
  ...
  const pool = poolSummary(loans, paymentsByLoan, deposits, transfers);
    ...
    <div className="flex flex-col gap-10">
      ...
      <div>
        <h2 className="mb-3 text-sm font-medium text-muted">
          Pool — <Link href="/lenden/pool" className="text-accent hover:opacity-80">view details</Link>
        </h2>
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
      ...
      <div>
        <h2 className="mb-1 text-sm font-medium text-muted">
          Earnings — <Link href="/lenden/reports" className="text-accent hover:opacity-80">view everyone</Link>
        </h2>
