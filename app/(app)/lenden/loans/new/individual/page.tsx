...
export default async function NewLoanPage() {
  ...
  if (profile?.role !== "owner" && profile?.role !== "contributor") redirect("/lenden");
  ...
  return (
    <div className="flex max-w-lg flex-col gap-8">
      <div>
        <Link href="/lenden/loans/new" className="mb-2 inline-block text-xs text-muted hover:text-foreground">
          ← Back
        </Link>
        <h1 className="text-xl font-semibold text-foreground">Funded by specific friends</h1>
        <p className="mt-1 text-sm text-muted">
          Principal is the sum of what each friend contributes below. They&apos;ll earn all the interest.
        </p>
      </div>

      {borrowers.length === 0 ? (
        <p className="text-sm text-muted">
          No borrowers yet — <Link href="/lenden/borrowers" className="text-accent">add one first</Link>.
        </p>
      ) : (
