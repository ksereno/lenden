...
import { getLoans, getAllPayments, getPoolDeposits, getPoolTransfers } from "@/lib/queries";
...
export async function createPoolLoan(formData: FormData) {
  ...
  if (principal > available) {
    redirect(`/lenden/loans/new/pool?error=insufficient&needed=${principal}&available=${available}`);
  }
  ...
  if (error || !loan) {
    redirect(`/lenden/loans/new/pool?error=create-failed&message=${encodeURIComponent(error?.message ?? "unknown error")}`);
  }

  redirect(`/lenden/loans/${loan.id}`);
}
